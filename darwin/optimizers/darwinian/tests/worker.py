...
from darwin.optimizers.darwinian.worker import optimize
from darwin.optimizers.python.worker import sha256_text
...
class DarwinianWorkerTest(unittest.TestCase):
    def test_fixture_result_contains_only_target_patch(self) -> None:
        previous = os.environ.get("ELLIOTT_DARWIN_FIXTURE")
        os.environ["ELLIOTT_DARWIN_FIXTURE"] = "1"
        try:
            result = optimize(request())
        finally:
            if previous is None:
                os.environ.pop("ELLIOTT_DARWIN_FIXTURE", None)
            else:
                os.environ["ELLIOTT_DARWIN_FIXTURE"] = previous
