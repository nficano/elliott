...
export class DarwinWireError extends Schema.TaggedErrorClass<DarwinWireError>()(
  "DarwinWireError",
  {
...
export const wireError = (
  message: string,
  status: DarwinWireError["status"] = HTTP_BAD_REQUEST,
): never => {
  throw DarwinWireError.make({ message, status });
};
...
export const errorResponse = (cause: unknown): Response => {
  if (cause instanceof DarwinWireError) {
