...
import { type DarwinServerConfig, makeDarwinFetchHandler } from "./http";
import {
  errorResponse,
  MAX_REQUEST_BYTES,
  readJsonRequest,
...
const start = () => {
  const config: DarwinServerConfig = {
    host: "127.0.0.1",
    port: 9073,
    maximumJobs: 1,
    token: "test-secret",
  };
  return makeDarwinFetchHandler(config, {
    "/v1/echo": (value) => value,
  });
};
...
describe("TypeScript darwin HTTP boundary", () => {
...
describe("TypeScript darwin wire boundary", () => {
  ...
  test("accepts only authenticated loopback darwin endpoints", () => {
