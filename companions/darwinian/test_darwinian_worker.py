...
from darwinian_worker import optimize
...
class DarwinianWorkerTest(unittest.TestCase):
    def test_fixture_result_contains_only_target_patch(self) -> None:
        ...
        self.assertEqual(candidate["runId"], "evr_12345678")
        self.assertEqual(candidate["validationScore"], 0)

    def test_digest_mismatch_fails_closed(self) -> None:
        value = request()
        value["codeSandbox"]["checkoutFiles"][0]["digest"] = "sha256:wrong"
        with self.assertRaisesRegex(ValueError, "digest mismatch"):
            optimize(value)

    def test_shell_executable_fails_closed(self) -> None:
        value = request()
        value["codeSandbox"]["testCommands"] = [["sh", "-c", "bun test"]]
