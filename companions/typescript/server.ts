...
export type CompanionHandler = (value: unknown) => unknown;

export interface CompanionRoutes {
  readonly [path: string]: CompanionHandler;
}

export type CompanionFetchHandler = (request: Request) => Promise<Response>;
...
class ConcurrencyGate {
  ...
  use = async <A>(operation: () => Promise<A>): Promise<A> => {
    if (this.#active >= this.maximum) {
      return wireError(
        "companion concurrency limit reached",
...
const requireLoopbackHost = (rawHost: string): "127.0.0.1" | "::1" => {
  if (rawHost !== "127.0.0.1" && rawHost !== "::1") {
    return wireError(
      "companion server must bind to loopback",
      HTTP_INTERNAL_SERVER_ERROR,
    );
  }
  return rawHost;
...
const resolveToken = (
  ...
  const fixture = environment["ELLIOTT_COMPANION_FIXTURE"] === "1";
  const token = environment["ELLIOTT_COMPANION_TOKEN"];
  if (!fixture && (token === undefined || token.length === 0)) {
    return wireError(
      "ELLIOTT_COMPANION_TOKEN is required outside fixture mode",
      HTTP_INTERNAL_SERVER_ERROR,
    );
  }
  return token ?? "";
...
export const loadCompanionServerConfig = (
  ...
  return {
    ...
    token: resolveToken(environment),
  };
};
...
export const makeCompanionFetchHandler = (
  config: CompanionServerConfig,
  routes: CompanionRoutes,
): CompanionFetchHandler => {
...
export const startCompanionServer = (
  ...
  Bun.serve({
    ...
    fetch: makeCompanionFetchHandler(config, routes),
  });
