...
- `skill` / `tool` scaffold a component package (manifest overlay, kind
  ...
  repository: `main.ts` (boot entry), `agents/<name>/agent.yaml`, a persona
  file, and `config/elliott.yaml` + `config/secrets.yaml` whose required LLM
  fields are env-backed placeholders — see
  [Configuration](configuration.md#required-configuration).
