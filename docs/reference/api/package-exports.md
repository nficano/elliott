...
The `elliott` package exposes these entry points (from `package.json`
`exports`). Import from the subpath, not from deep file paths.

| Import                  | Module                              | Contents                                              |
| :---------------------- | :---------------------------------- | :---------------------------------------------------- |
| `elliott`               | `src/index.ts`                      | top-level surface incl. `AgentKernel`                 |
...
| `elliott/providers`     | `src/providers/index.ts`            | first-party model providers (LiteLLM, Ollama)         |
| `elliott/manifest`      | `src/manifest/index.ts`             | Markdown / YAML / SKILL.md parsing & hardening, scaffolding |
| `elliott/mcp`           | `src/mcp/index.ts`                  | MCP endpoints, exposure, drivers                      |
| `elliott/gateway`       | `src/gateway/index.ts`              | message source/sink pipelines, identity & session     |
| `elliott/memory`        | `src/memory/index.ts`               | session-store, curated, external-slot providers       |
...
| `elliott/skills`        | `src/runtime/skills/index.ts`       | the skill `register()` seam (see [skill-context.md](skill-context.md)) |
| `elliott/runtime`       | `src/runtime/types.ts`              | runtime type surface (`ToolDefinition`, settings, …)  |
| `elliott/runtime/app`   | `src/runtime/app.ts`                | the production application                            |
| `elliott/package.json`  | —                                   | package metadata                                      |

The `elliott` binary (`bin.elliott` → `src/cli.ts`) is documented in
[../cli.md](../cli.md).
