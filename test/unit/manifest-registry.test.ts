...
describe("M2 manifests and discovery", () => {
  ...
  it("loads overlays, array tool lists, and rejects untrusted or unclosed skills", () => {
    expect(() =>
      loadAgentSkill({
        namespace: "workspace",
        markdown: "---\nname: broken\n",
    ...
    expect(skill.allowedTools).toEqual(["Bash"]);
    expect(
      skill.requestedCapabilities.some((request) =>
        request.capability === "files.read" && request.deferred === true
      ),
    ).toBe(true);
  });
