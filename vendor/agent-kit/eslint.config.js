...
// dprint typescript config — mirrors the repo's existing 2-space / double-quote /
// semicolon style so adopting the formatter is not a mass reflow. The ESLint
// `@effect/dprint` rule formats via the bundled @dprint/typescript engine; the
// `dprint` CLI (format scripts) reads dprint.json with the same values.
const dprintConfig = {
  indentWidth: 2,
  lineWidth: 80,
  semiColons: "always",
...
export default tseslint.config(
  ...
  // Turn off every core stylistic rule that would fight dprint (the plugin's own
  // preset). This is the "remove conflicting rules" pass on the formatting axis.
  ...effectDisableConflicts,
  {
    ...
    languageOptions: {
      parserOptions: {
        // Type-aware linting; allowDefaultProject lets this config file (which no
        // tsconfig includes) resolve via the default project.
        projectService: {
    ...
    rules: {
      ...
      // import hygiene. NOTE: `import-x/order`'s autofixer conflicts destructively
      // with the dprint fixer under editor fix-all-on-save (it duplicated/dropped
      // imports) — left off so the two formatters don't race. `no-cycle` is not
      // autofixable and stays on (it's the load-bearing one).
      "import-x/order": "off",
      ...
      // Effect uses idiomatic short names (ctx, err, ref, cfg) and interops with
      // null; unicorn's rename/no-null opinions conflict with that — removed.
      "unicorn/name-replacements": "off",
      "unicorn/prevent-abbreviations": "off",
      "unicorn/no-null": "off",
      // Functional schema/Effect code is intentionally call- and closure-heavy;
      // these style rules obscure the declarative structure instead of helping.
      ...
      // Interface implementations often need Promise-returning methods even when
      // a particular implementation completes synchronously.
      ...
      // The TypeScript ESLint API still exposes this supported compatibility API.
      "sonarjs/deprecation": "off",
      // Its autofix rewrites `() => undefined` to a void `() => {}`, which changes
      // a value-returning callback's type — disabled to keep fix-on-save safe.
  ...
  {
    files: [
      "src/host/config/interpolate.ts",
      "src/store/migrate.ts",
    ],
    rules: {
      "no-restricted-syntax": ["error", timeValueRestriction],
    },
  },
  {
    files: ["test/**/*.ts"],
