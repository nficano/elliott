...
smoke() {
  ...
  docker run \
    ...
    --memory 1g \
    --cpus 1 \
    --env ELLIOTT_DARWIN_FIXTURE=1 \
    --env ELLIOTT_DARWIN_TOKEN=smoke-secret \
  ...
  docker exec "${container}" \
    bun /opt/elliott/darwin/runtime/smoke.ts \
...
smoke \
  ...
  9071 \
  darwin/optimizers/dspy/fixtures/request.json \
  optimizer \
  /v1/optimize
smoke \
  darwinian \
  ...
  9072 \
  darwin/optimizers/darwinian/fixtures/request.json \
  optimizer \
  /v1/optimize
smoke \
  benchmarks \
...
smoke \
  ...
  elliott/evaluator-agent-benchmarks:local \
  9073 \
  darwin/evaluators/agent-benchmarks/fixtures/code-check.json \
  code-check \
  /v1/candidate/check
