...
// Pure parse of a `benchmark:<ref>#<task-id,...>` source specification: slices
// the prefix, splits ref from tasks, trims/drops empty task ids, validates the
// ref, and defaults to ["bootstrap"] when no tasks are named. The Effect shell
// maps the error branch to EvolutionDatasetError and does the digest/effect work.
const BENCHMARK_SOURCE_PREFIX = "benchmark:";

export const parseBenchmarkSourceSpecification = (
  spec: string,
):
  | { readonly benchmarkRef: string; readonly taskIds: readonly string[]; }
  | { readonly error: string; } =>
{
  const specification = spec.slice(BENCHMARK_SOURCE_PREFIX.length);
  ...
  if (benchmarkRef === undefined || benchmarkRef.length === 0) {
    return { error: "benchmark source requires benchmark:<ref>#<task-id,...>" };
  }
  return {
    benchmarkRef,
    taskIds: taskIds.length > 0 ? taskIds : ["bootstrap"],
  };
};
...
const benchmarkSource = (
  ...
  const parsed = parseBenchmarkSourceSpecification(source);
  if ("error" in parsed) {
  ...
  return Effect.succeed(benchmarkDerivedDatasetSource({
    benchmarkRef: parsed.benchmarkRef,
