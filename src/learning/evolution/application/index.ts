...
// Pure review-quorum decision: encodes the self-approval and duplicate-reviewer
// guards, approver accumulation, and the approved (threshold crossed) vs
// awaiting-review outcome. The Effect shell maps guard violations to the same
// errors and performs the store update plus metric recording.
export const decideProposalOutcome = (
  proposal: Proposal,
  ...
  if (status === "approved" && proposal.author === reviewer) {
    return { kind: "guard-violation", violation: "self-approval" };
  }
  if (status === "rejected") {
    ...
    return { kind: "decided", updated: rejected, decision: "rejected" };
  }
  const currentApprovers = proposalApprovers(proposal);
  if (currentApprovers.includes(reviewer)) {
    return { kind: "guard-violation", violation: "duplicate-reviewer" };
  }
  const approvers = Object.freeze([...currentApprovers, reviewer]);
  const approved = approvers.length >= requiredReviewCount;
  ...
  return approved
    ? { kind: "decided", updated, decision: "approved" }
    : { kind: "decided", updated };
};
...
const decideProposal = async (
  ...
  const outcome = decideProposalOutcome(
    proposal,
    reviewer,
    status,
    requiredEvolutionReviewCount(proposal.evolution),
  );
  if (outcome.kind === "guard-violation") {
    throw new TypeError(
      outcome.violation === "self-approval"
        ? "Proposal author cannot approve their own Proposal"
        : "Proposal reviewer already recorded a decision",
    );
  }
  await persistProposalDecision(input, outcome.updated, outcome.decision);
