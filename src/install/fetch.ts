...
// Assert a parsed manifest identifies the expected skill and declares a known
// document kind. Pure: throws InstallError on the first violation with the same
// messages the loader relies on. The caller supplies the parsed manifest so
// readFile/parse stay in the async shell.
...
const validateSkillTree = async (
  ...
  validateSkillManifest(parse(manifestRaw), name, version);
