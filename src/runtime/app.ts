...
// Pure route dispatch for the runtime HTTP handler. Precedence is fixed: health,
// components, the two control-plane paths (each only when its capability flag is
// set), a dynamic route-table match on method+path, else not-found — so a
// control-plane path whose capability is absent falls through to the route table
// (and usually 404). `#handleRequest` switches on the result to build responses.
export const resolveRuntimeRoute = (
  method: string,
  pathname: string,
  opts: {
    readonly hasEvolutionControlPlane: boolean;
    readonly hasGovernanceControlPlane: boolean;
    readonly routes: readonly RouteBinding[];
  },
): RuntimeRouteResolution => {
  if (pathname === "/healthz") return { kind: "health" };
  if (pathname === "/v1/components") return { kind: "components" };
  if (pathname === "/v1/control/evolution" && opts.hasEvolutionControlPlane) {
    return { kind: "evolution-control" };
  }
  if (pathname === "/v1/control/governance" && opts.hasGovernanceControlPlane) {
    return { kind: "governance-control" };
  }
  const index = opts.routes.findIndex(
    (item) => item.method === method && item.path === pathname,
  );
  if (index !== -1) return { kind: "route", index };
  return { kind: "not-found" };
};

// The primary gateway is the one used for unsolicited delivery: prefer a gateway
// that can both send and knows a default channel, else any gateway that can send.
export const selectPrimaryGateway = (
  gateways: readonly GatewayBinding[],
): GatewayBinding | undefined =>
  gateways.find(
    (gateway) =>
      gateway.send !== undefined && gateway.defaultChannel !== undefined,
  ) ?? gateways.find((gateway) => gateway.send !== undefined);

// Where a reply goes: the originating gateway when it can send, else the primary.
export const selectReplyGateway = (
  gateways: readonly GatewayBinding[],
  originName: string,
): GatewayBinding | undefined => {
  const origin = gateways.find((gateway) => gateway.name === originName);
  return origin?.send === undefined ? selectPrimaryGateway(gateways) : origin;
};
...
export class ElliottRuntime {
  ...
  #primaryGateway(): GatewayBinding | undefined {
    return selectPrimaryGateway(this.#gateways);
  }

  #replyGateway(message: InboundMessage): GatewayBinding | undefined {
    return selectReplyGateway(this.#gateways, message.gateway);
  }
  ...
  async #handleRequest(
    ...
    const resolution = resolveRuntimeRoute(request.method, url.pathname, {
      hasEvolutionControlPlane: this.#evolutionControlPlane !== undefined,
      hasGovernanceControlPlane: this.#governanceControlPlane !== undefined,
      routes: this.#routes,
    });
    switch (resolution.kind) {
      ...
      case "route": {
        return this.#routes[resolution.index]!.handle(request, events);
      }
      case "not-found": {
