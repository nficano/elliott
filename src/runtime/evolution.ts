...
// Pure parse of an engine resume token (`<route>:<token>`). Returns the routed
// engine and the bare token, or an error reason when the separator is missing or
// the route prefix is unrecognized. The engine dispatches on the result.
export const parseEvolutionResumeToken = (
  resumeToken: string,
): { route: "text" | "code"; token: string; } | { error: string; } => {
  const separator = resumeToken.indexOf(":");
  if (separator === -1) {
    return { error: "resume token has no engine route" };
  }
  const route = resumeToken.slice(0, separator);
  if (route !== "text" && route !== "code") {
    return { error: "resume token has an unknown engine route" };
  }
  return { route, token: resumeToken.slice(separator + 1) };
};
...
const makeRuntimeOptimizationEngine = (
  ...
  resume: (resumeToken) => {
    const parsed = parseEvolutionResumeToken(resumeToken);
    if ("error" in parsed) {
