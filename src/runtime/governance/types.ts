...
// Parsed /v1/control/governance POST body. `parseGovernanceOperation` derives it
// from the raw JSON; the control-plane shell switches on `kind` and calls the
// governor. An unrecognized shape becomes `invalid`, carrying the operator-facing
// error message to return.
export type GovernanceOperation =
  | { readonly kind: "freeze"; }
  | { readonly kind: "unfreeze"; }
  | { readonly kind: "disable"; readonly tool: string; }
  | { readonly kind: "enable"; readonly tool: string; }
