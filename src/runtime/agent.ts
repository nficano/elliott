...
export class RuntimeAgent {
  ...
  #systemPrompt(conversation: string): string {
    ...
    return buildSystemPrompt(persona, new Date().toISOString());
  }
...
// Pure assembly of the system prompt: the (cached) persona, the fixed runtime
// security framing, and the current time injected by the caller so this stays
// deterministic.
export const buildSystemPrompt = (persona: string, isoTime: string): string =>
