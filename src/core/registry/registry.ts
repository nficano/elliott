...
const isPinned = (entries: readonly RegistryEntry[]): boolean =>
  entries.some((entry) =>
...
/** Pure scope-precedence resolution with the org-pinning shadow rule (§7.5).
 *  Empty input resolves to undefined; a narrower scope shadowing a pinned or
 *  security-critical component throws {@link PinnedShadowError}. */
export const resolveEntries = (
  ref: ComponentRef,
  entries: readonly RegistryEntry[],
): RegistryResolution | undefined => {
  if (entries.length === 0) return undefined;
  if (isPinned(entries)) return resolvePinned(ref, entries);
...
export class ComponentRegistry {
  ...
  resolve(ref: ComponentRef): RegistryResolution | undefined {
    return resolveEntries(ref, this.#entries.get(ref) ?? []);
  }
