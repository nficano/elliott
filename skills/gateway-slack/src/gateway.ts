...
export class SlackGateway implements GatewayBinding {
  ...
  // "connected" only proves the socket; event recency proves Slack is still
  // delivering. A live workspace goes quiet when the app's event
  // subscriptions are edited away — the socket stays green while the agent
  // is deaf (this happened 2026-07-29) — so /healthz surfaces both.
  status(): string {
    const base = this.#connected ? "connected" : "connecting";
    if (this.#lastEventAt === undefined) {
      return `${base} (no events since boot)`;
    }
    const minutes = Math.floor(
      (this.#now() - this.#lastEventAt) / MILLISECONDS_PER_MINUTE,
    );
    return `${base} (last event ${minutes}m ago)`;
  }
