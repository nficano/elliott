...
- `GET /v1/deeptrace/topology` — the connection graph (nodes/edges):
...
- `GET /v1/deeptrace/state` — current snapshot (turns, db stats, events).
- `GET /v1/deeptrace/stream` — Server-Sent-Events live feed.
- `GET /v1/deeptrace/turn?id=<runId>` — one turn's full event detail.
