...
import type { YouTubeOAuthSettings } from "../../../src/runtime/types";
import type { AccessTokenSource } from "./types";

// YouTube Data API access via an OAuth refresh token (scope youtube.force-ssl).
// The refresh credentials are broker-managed and never enter model context; we
// only cache the short-lived access token in memory.
const TOKEN_URL = "https://oauth2.googleapis.com/token";
const MILLISECONDS_PER_SECOND = 1000;
const EXPIRY_SKEW_SECONDS = 60;
const DEFAULT_TTL_SECONDS = 3600;

export const makeTokenSource = (
...
const mintAccessToken = async (
  ...
  if (!response.ok) {
    throw new Error(`YouTube token endpoint returned HTTP ${response.status}`);
  }
  const payload: unknown = await response.json();
  if (!isJsonRecord(payload) || typeof payload["access_token"] !== "string") {
    throw new Error("YouTube token endpoint returned an invalid payload");
  }
